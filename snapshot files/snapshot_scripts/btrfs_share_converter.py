...
def ConvertShares(shareName):
    convertingAll = False
    if shareName == None or shareName == '':
    ...
    if shareName.lower() == "-all":
        ...
        s = os.path.basename(shareName)# just in case the full path is passed through
        s = s.replace('/', '')
        shareList.append(s)
